...
template <typename T>
CMyList<T>::CMyList(const CMyList& CMyList) // копируем данные, создаем новые указатели
{
...
template<typename T>
CMyList<T>::CMyList(CMyList&& CMyList) // копируем указатели на начало и конец, забираем value у исходного файла удаляем начало и конец.
{
...
template <typename T>
CMyList<T>& CMyList<T>::operator=(const CMyList& copy)
{
	if (this != &copy)
	{
		CMyList tmp(copy);
...
template <typename T>
CMyList<T>& CMyList<T>::operator=(CMyList&& move)
{
...
template <typename T>
size_t CMyList<T>::GetLength() const
{
	return m_length;
}
...
template <typename T>
void CMyList<T>::Insert(const Iterator& it, const T& value)
{
	if (m_first == nullptr)
	{
		PushBack(value);
...
template <typename T>
void CMyList<T>::Erase(Iterator& it)
{
...
template <typename T>
CMyList<T>::ConstIterator CMyList<T>::begin() const
{
	return { m_first, m_length, 0 };
}

template <typename T>
CMyList<T>::ConstIterator CMyList<T>::end() const
{
	return { nullptr };
}

template <typename T>
CMyList<T>::ReverseIterator CMyList<T>::rbegin()
{
...
template <typename T>
CMyList<T>::ReverseIterator CMyList<T>::rend()
{
	return std::make_reverse_iterator(this->begin());
}

template <typename T>
CMyList<T>::ConstReverseIterator CMyList<T>::rсbegin() const
{
...
template <typename T>
CMyList<T>::ConstReverseIterator CMyList<T>::rсend() const
{
	return std::make_reverse_iterator(this->begin());
}